...
export default function NotesModal({
  ...
  const fetchNotes = async () => {
    ...
    try {
      ...
      if (error) {
        console.error("[NotesModal] Error fetching notes:", error);
        toast.error("Erreur lors du chargement des notes");
        return;
      }
      
      setNotes(data || []);
      console.log("[NotesModal] Notes loaded:", data?.length || 0);
    } catch (error) {
      console.error("[NotesModal] Exception fetching notes:", error);
  ...
  }, [open, projectId]); // Removed fetchNotes from dependencies to prevent infinite loops
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing && editingNote) {
        ...
        console.log('[DEBUG] Calling createNote API with data:', {
          ...noteData,
        ...
        console.log('[DEBUG] createNote API response:', {
          success: !!data,
          hasError: !!error,
        ...
        if (error) {
          console.error('[DEBUG] Create note error details:', {
            error,
      ...
      console.error("[NotesModal] Error saving note:", error);
  ...
  const handleDelete = async (noteId: string) => {
    ...
    try {
      ...
      console.error("[NotesModal] Error deleting note:", error);
  ...
  const handleToggleActive = async (noteId: string, isActive: boolean) => {
    try {
      ...
      console.error("[NotesModal] Error toggling note:", error);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[90vw] max-h-[85vh] flex flex-col rounded-2xl bg-card shadow-none border-none">
        <DialogHeader>
